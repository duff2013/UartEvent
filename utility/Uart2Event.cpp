...
volatile int Uart2Event::rxTermCharacter;
...
void Uart2Event::serial_dma_rx_isr( void ) {
    ...
    if ( term != -1 ) {
        ...
        if ( current == term ) NVIC_SET_PENDING( IRQ_UART1_STATUS );
    } else NVIC_SET_PENDING( IRQ_UART1_STATUS );
    *elink = 1;
...
void Uart2Event::serial_dma_begin( uint32_t divisor ) {
    ...
    attachInterruptVector( IRQ_UART1_STATUS, user_isr );
